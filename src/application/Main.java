...
public class Main extends Application {
	...
	public void start(Stage stage) {
		try {
			...
			video.setText("Video");
			video.setStyle("-fx-font-size: 15; -fx-text-fill: Black;");
			
			//Setup and add buttons to control bar
			controlBar = new HBox();
			controlBar.setSpacing(20);
			controlBar.getChildren().addAll(next, prev, slideShow, imgGal, video);
			...
			video.setOnAction(e->{
				...
				Player vidplayer  = new Player(canvas,20,50, stage, scene);
				String [] paths = {
						"1280.mp4"
				};
				...
				Scene sceneVid = new Scene(vidplayer,vidplayer.getWindowWidth(),vidplayer.getWindowHeight());
				//stage.setX(vidplayer.getXScreenPosition());
				//stage.setY(vidplayer.getYScreenPosition());
				//stage.setFullScreen(false);	
				stage.setScene(sceneVid);
				//stage.show();
...
//			});
			...
			gridGalleryPane.setOnMouseClicked(e->{
				Node node  = e.getPickResult().getIntersectedNode();
				if (node != gridGalleryPane) {
			        node.setStyle("-fx-background-color: Black; -fx-padding: 10;");
			    }
			});
		}catch(Exception e) {
			e.printStackTrace();
